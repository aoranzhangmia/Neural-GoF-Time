...
class KSD(object):
    ...
    def kappa(self, X, Y, method="direct"):
        ...
        k = self.kernel(X, Y)
        # \sum_{x\in X} K(X\x, Y)
        k_X = sum(self.kernel(np.delete(X, i, axis=0), Y) for i in range(n))
        # \sum_{y\in Y} K(X, Y\y)
        k_Y = sum(self.kernel(X, np.delete(Y, j, axis=0)) for j in range(m))
    ...
    def kappa_mmd(self, X, Y, debug=False):
        ...
        # \sum_{x\in X} K(X\x, Y)
        k_X = sum(k_x(i) for i in range(n))
        # \sum_{y\in Y} K(X, Y\y)
        k_Y = sum(k_y(j) for j in range(m))
        # \sum_{x,y\in Y} K(X\x, Y\y)
        k_X_Y = sum(k_x_y(i, j) for i in range(n) for j in range(m))

        # Cache Papangelou computations
        cu = [self.papangelou(U[u], X) for u in range(npts)]
        cv = [self.papangelou(V[v], Y) for v in range(npts)]
        ...
        def integrand_v(v):  # Integrand over v
            k_Xv = sum(k_xv(i, v) for i in range(n))  # \sum_x k(X\x, Y + v)
            return ((k_Xv - k_X) - n*(kv(v) - k)) * cv[v]

        def integrand_u(u):  # Integrand over u
            ku_Y = sum(ku_y(u, j) for j in range(m))  # \sum_y k(X + u, Y\y)
            return ((ku_Y - k_Y) - m*(ku(u) - k)) * cu[u]

        if self.int_method == 'trapz':
            if self.dim == 1:
                term1 = h**2. * sum(integrand_uv(u, v) for u in range(npts)
                                    for v in range(npts))
                term2 = h * sum(integrand_v(v) for v in range(npts))
                term3 = h * sum(integrand_u(u) for u in range(npts))

            elif self.dim == 2:
                term1 = h0**2 * h1**2 * sum(integrand_uv(u, v)
                                            for u in range(npts)
                                            for v in range(npts))
                term2 = h0 * h1 * sum(integrand_v(v) for v in range(npts))
                term3 = h0 * h1 * sum(integrand_u(u) for u in range(npts))

        elif self.int_method == 'monte_carlo':
            term1 = np.mean([integrand_uv(_, _) for _ in range(npts)])
            term2 = np.mean([integrand_v(_) for _ in range(npts)])
            term3 = np.mean([integrand_u(_) for _ in range(npts)])
